...
(function () {
 ...
 els.empty.innerHTML = `<span></span> Couldn't generate your travel report: ${err.message}. Please try again.`;
 }
 }
 ...
 els.askMore.classList.remove('active');
 els.empty.style.display = 'block';
 els.empty.innerHTML = '<span></span>Select any marked date on the calendar to generate your AI travel report.';
 });

 document.getElementById('atc-btn-book').addEventListener('click', () => {
 const place = lastSelection ? lastSelection.location : '';
 const dateStr = lastSelection ? lastSelection.date : '';
 const params = new URLSearchParams({ place, region: place, date: dateStr });
