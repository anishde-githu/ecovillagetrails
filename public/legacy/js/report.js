...
function render(data) {
 ...
 <div class="book-stay-row">
 <a class="book-stay-btn" href="book.html?place=${encodeURIComponent(destination)}&region=${encodeURIComponent(destination)}">
 Book Your Stay &amp; Travel
 </a>
 </div>
 ...
 <div class="icon"></div><div class="title">Tour Map</div><div class="sub">Open in Google Maps</div>
 </div>
 <div class="action-card" onclick="window.open('https://wttr.in/${weatherQuery}','_blank')">
 <div class="icon"></div><div class="title">Weather</div><div class="sub">Live forecast</div>
 </div>
 ...
 <div class="icon"></div><div class="title">Generate PDF</div><div class="sub">Download this report</div>
 </div>
 </div>
...
(function () {
 ...
 const reportData = sessionStorage.getItem('ecoTripReport');
 ...
 const payload = {
 message: q,
 formData: reportData ? JSON.parse(reportData).formData : null,
 };
 const res = await fetch('/api/chat', {
 method: 'POST',
 headers: { 'Content-Type': 'application/json' },
 ...
 if (data.success) addBubble(data.reply, false);
